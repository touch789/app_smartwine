...
final databaseReference = FirebaseDatabase.instance.reference().child("Bottle/2/data");
...
class _HomePageState extends State<HomePage> {
  ...
  void readData(){
    databaseReference.orderByKey().limitToFirst(5).once().then((DataSnapshot snapshot) {
  ...
  _showDialog() async {
    DocumentSnapshot variable = await Firestore.instance.collection('users').document(widget.uid).collection('compteur').document('count').get();
            ...
            FlatButton(
                ...
                onPressed: () {
                  if (BottleTitleInputController.text.isNotEmpty) {
                    Firestore.instance.collection('users').document(widget.uid).collection('compteur').document('count').setData(
                        {"count":(nb+1).toString()});
                    Firestore.instance
                        .collection("users")
                        .document(widget.uid)
